...
- **In-Browser Text Transforms & Structure Tools**: Highlight text in any web field and select an AI transform from the Tone popover or popup picker. In addition to stylistic rewrites (Professional, Casual, Friendly, Formal, Academic, Concise), the extension now includes Structure tools (**Summary**, **Key Points**, and **List**) operating at calibrated low temperature (`0.2`) to distill paragraphs or reformat prose into clean Markdown bullet points.
...
- **Virtual Editor & Helper Textarea Shield (`isHelperTextarea`)**: Detects and ignores hidden virtual editor input sinks (such as Monaco editor inputs, `aria-hidden` helper textareas, and specialized code editor wrappers). This prevents web pages (including job application portals, dev environments, and CMS interfaces) from freezing or crashing into blank white screens.
- **Pause Proofreading & Per-Site Disable (Online or Offline)**: Extension popup toggles to pause proofreading globally (rewrites stay available) or disable Lexicon on the current site, with settings persisted in extension storage. The pause toggle functions reliably even when the desktop app or backend sidecar is closed or unreachable.
- **Robust In-Page DOM Replacement**: Enhanced `editLanded` confirmation logic to properly accommodate multiline text and rich DOM block boundaries (such as paragraphs in Notion or Slack/Discord composers) without reporting false-positive rejection notices.
...
- **Modular Compute Acceleration & Runtime Management (`gpu_manager.py`, `HardwareTab.jsx`)**: Added a modular acceleration package system allowing users to download, install, activate, and uninstall hardware runtimes on demand. Supports NVIDIA CUDA 12.4 (`v0.3.34-cu124`) for Tensor Core acceleration on RTX 2000–5000 series GPUs and Vulkan 1.3+ (`v0.3.34-vulkan`) for AMD Radeon, Intel Arc, and modern processor graphics across 98%+ of consumer and professional GPU hardware.
- **Dynamic In-Process Engine Reloading**: Seamlessly switches between CUDA, Vulkan, and CPU backends in real time without restarting the desktop application. Automatically configures DLL library search paths (`ggml-cuda.dll`, `ggml-vulkan.dll`, `llama.dll`) and re-initializes the bundled `llama.cpp` runtime.
- **Interactive Runtime Switching & Package Deletion**: Added animated indicators and responsive feedback when switching acceleration runtimes or deleting acceleration packs, ensuring immediate UI state synchronization with no lingering action buttons.
- **Clear Dual-Level Explanation for Limit Model Offload**: Non-technical summary ("Keeps AI model layers strictly inside your dedicated GPU memory (VRAM) so slower system RAM is not used") paired with technical details on avoiding CPU/system memory layer splitting.
- **Runtime Version Transparency**: Explicitly displays exact engine runtime versions (llama.cpp v0.3.34, CUDA 12.4, Vulkan 1.3+) and target hardware compatibility directly in the Hardware settings.
...
- **Desktop Autostart Persistence & Startup Hygiene**: Added an option to start Lexicon automatically with the operating system, minimizing to the system tray for instant background availability. Autostart preference is reliably preserved across application updates, cleanly registers in Windows/terminal autostart lists, and removes lingering debug registry entries on boot.
...
- **Honest Tier Switching & Instant Model Loading**: Picking a downloaded tier now shows an explicit "Switching to…" state, immediately unloads the previous model from memory, and loads the newly selected model into RAM/VRAM right away. System resource monitors and UI telemetry accurately reflect the active model in real time rather than waiting for an AI generation task.
- **Explicit Memory Residency Controls**: Added "Load into memory" and "Unload from memory" buttons in Settings, enabling users to pre-load model weights into VRAM/RAM or manually free memory at any time.
- **Concise Active Status Indicators**: Replaced lengthy "loaded into memory" labels with a compact `● Active` (green dot) indicator when loaded versus `○ Installed` (gray dot) when idle on disk, keeping the interface clean and preventing text from running off screen.
- **Autonomous 5-Minute Auto-Unload & Native AI Activity Monitor (`inference.py`)**: A Python-native background daemon monitors actual AI operations (`complete()`, `load_bundled_model()`) and automatically calls `unload_active_backend()` after 300 seconds (5 minutes) of inactivity, releasing GPU VRAM and system RAM across desktop and dev environments.
- **Passive Polling Guard for Desktop Idle Monitor (`main.rs`, `api.js`)**: Fixed an issue where the frontend's 10-second background dictionary sync (`/dictionary`) invoked `ensure_backend` and continuously reset Tauri's idle activity timer and offload flags every 10 seconds. Passive polling (`/dictionary`, `/ai/status`, `/model/status`) is now explicitly separated from active user actions, allowing the 5-minute auto-unload to function properly.
- **Live Residency Polling in Settings (`ModelManager.jsx`)**: When a model is loaded in memory, the Settings view polls status in the background so that when the 5-minute timeout fires, the UI automatically flips from `● Active` to `○ Installed` and updates the action button to `Load into memory` without requiring a manual refresh or click.
- **Idle Unload Setting Notice**: Settings dialog explicitly informs users that active models automatically unload from memory after 5 minutes of inactivity to free GPU and system RAM.
